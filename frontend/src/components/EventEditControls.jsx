...
import {HiArrowDown, HiArrowUp, HiPhoto, HiPlus, HiTrash} from "react-icons/hi2";
...
const EventEditControls = ({event, onChange, onSubmit, onAddNewImage, className = ""}) => {
    ...
    const handleAddHighlight = (e) => {
        e.preventDefault(); // This is already here, but ensuring it's properly preventing the default behavior
        const value = highlightInput.trim();
        if (value && !highlights.includes(value)) {
            const newHighlights = [...highlights, value];
    ...
    return (
        <div className={`flex flex-col gap-6 w-full ${className} text-slate-300`}>
            ...
            <div className="flex flex-col gap-1.5">
                ...
                <div className="flex flex-wrap gap-2">
                    {highlights.map((highlight, idx) => (
                        <span key={idx}
                              className="flex items-center gap-1.5 bg-sky-600 text-white rounded-full px-3 py-1 text-sm font-medium">
              {highlight}
                            <button
                                type="button"
                ...
                <HiTrash className="h-4 w-4"/>
                    ...
                    <div className="flex gap-2 w-full max-w-md">
                        ...
                        <button
                            type="button"
                            className="interactive-element px-4 py-2 bg-sky-600 hover:bg-sky-700 text-white rounded-lg shadow font-medium text-sm transition-all duration-200"
                ...
                <span className="text-xs text-slate-400">Press enter to add highlight. Click ✕ to remove.</span>
            </div>
            <div className="flex flex-col gap-2">
                ...
                <ul className="space-y-4">
                    {event.descriptions && event.descriptions.length > 0 ? event.descriptions.map((desc, idx) => (
                        <li key={idx}
                            className="group flex flex-col gap-2 p-4 bg-slate-700/50 border border-slate-600 rounded-lg">
                            <div className="flex flex-wrap gap-2 items-center">
                                ...
                                <div className="flex gap-1.5 flex-shrink-0">
                                    <button
                                        type="button"
                                        className="interactive-element p-2 bg-slate-600 hover:bg-slate-500 text-sky-300 rounded-md text-xs transition-all disabled:opacity-50"
                                        onClick={() => {
                                            if (idx > 0) {
                                    ...
                                    <button
                                        type="button"
                                        className="interactive-element p-2 bg-red-700 hover:bg-red-600 text-white rounded-md text-xs transition-all"
                                        onClick={() => {
                ...
                <button
                    type="button"
                    className="interactive-element mt-2 px-4 py-2 bg-sky-600 hover:bg-sky-700 text-white rounded-lg shadow font-medium text-sm transition-all duration-200 flex items-center gap-1.5 self-start"
                    onClick={() => {
                        const descs = [...(event.descriptions || [])];
            ...
            <div className="flex flex-col gap-2">
                ...
                <ul className="space-y-4">
                    {event.images && event.images.length > 0 ? event.images.map((img, idx) => (
                        <li key={img.id || idx}
                            className="group flex items-center gap-4 p-3 bg-slate-700/50 border border-slate-600 rounded-lg">
                            <div className="relative flex-shrink-0">
                                ...
                                />
                                <span
                                    className="absolute -top-2 -left-2 bg-slate-800 text-sky-400 text-xs font-bold px-1.5 py-0.5 rounded-full border border-slate-500 shadow">{idx + 1}</span>
                            </div>
                            <div className="flex flex-col flex-1 gap-2 items-start">
                                ...
                                <span className="text-xs text-slate-400 mt-1 truncate max-w-[150px]"
                                      title={img.url || img.name || `Image ${idx + 1}`}>{img.url?.substring(img.url.lastIndexOf('/') + 1) || img.name || `Image ${idx + 1}`}</span>
                            </div>
                        </li>
                    )) : <span className="text-slate-400 italic text-sm">No images uploaded. Click below to add new images.</span>}
                </ul>
                <label
                    className="mt-3 w-full p-6 flex flex-col items-center justify-center border-2 border-dashed border-slate-600 rounded-lg bg-slate-700/50 text-sky-400 cursor-pointer hover:bg-slate-700 hover:border-sky-500 transition shadow-inner interactive-element">
            ...
            <div className="flex items-center gap-3 p-4 bg-slate-700/50 border border-slate-600 rounded-lg"><input
                id="event-featured"
                type="checkbox"
                checked={!!event.featured}
                onChange={e => onChange('featured', e.target.checked)}
                className="interactive-element w-5 h-5 accent-sky-500 bg-slate-600 border-slate-500 rounded focus:ring-2 focus:ring-offset-0 focus:ring-offset-slate-800 focus:ring-sky-500 cursor-pointer"
            />
            ...
            <button
                className="interactive-element w-full px-6 py-3 bg-sky-600 hover:bg-sky-700 text-white rounded-lg font-semibold text-base shadow-md transition-all duration-200 disabled:opacity-70 flex items-center justify-center gap-2 mt-2"
                onClick={onSubmit}
            >
                Submit Changes
            </button>
