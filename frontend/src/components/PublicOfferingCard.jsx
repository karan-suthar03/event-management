        ...
        <div className={`${cardBaseClass} ${modernCardSpecificClass}`}>
                ...
                <div className="h-40 w-full overflow-hidden rounded-t-xl">
                    <img
                        ...
                        onError={(e) => {
                            ...
                            e.target.classList.add('p-6', 'object-contain', 'bg-gray-700');
                        }}
                    />
                </div>
                {offering.bestSeller && (
                    <span
                        className={`absolute top-2 right-2 px-2.5 py-1 text-xs font-bold rounded-full shadow-md ${modern ? 'bg-sky-500 text-sky-100' : 'bg-yellow-300 text-yellow-800'}`}>
            Best Seller
          </span>
                )}
            </div>
            <div className={`p-4 flex flex-col flex-grow ${modern ? 'text-slate-300' : ''} overflow-y-auto`}>
                ...
                {offering.description &&
                    <p className={`text-xs mb-2 ${modern ? 'text-slate-400' : 'text-gray-600'} flex-shrink-0`}>{offering.description}</p>}
                <div
                    className={`text-sm font-semibold mb-1 ${modern ? 'text-sky-300' : 'text-pink-600'} flex-shrink-0`}>Approx.
                    Price: ₹{displayPrice}
                ...
                {Array.isArray(offering.inclusions) && offering.inclusions.length > 0 && (
                    <div className="mt-auto pt-2 border-t ${modern ? 'border-slate-700' : 'border-pink-100'}">
                        <h4 className={`text-sm font-semibold my-2 ${modern ? 'text-slate-300' : 'text-pink-600'}`}>What's
                            Included:</h4>
