...
const RequestOfferingModal = ({offering, onClose, modern = false}) => {
                ...
                >
                    ...
                    {success ? (
                        <div className="text-center py-8">
                            <svg xmlns="http://www.w3.org/2000/svg" className="h-16 w-16 text-green-400 mx-auto mb-4"
                                 fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2">
                                <path strokeLinecap="round" strokeLinejoin="round"
                                      d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"/>
                            </svg>
                            <p className="text-xl font-semibold text-green-300 mb-2">Request Sent Successfully!</p>
                            <p className="text-gray-400">Ananya will get in touch with you shortly regarding your
                                request for "{offering?.title}".</p>
                            <button
                                onClick={onClose}
                        ...
                        <form className="flex flex-col gap-4 sm:gap-5" onSubmit={handleSubmit}>
                            <div>
                                <label htmlFor="name" className="block text-sm font-medium text-gray-300 mb-1">Your
                                    Name</label>
                                <input
                                    id="name"
                                    name="name"
                            ...
                            <div>
                                <label htmlFor="contact" className="block text-sm font-medium text-gray-300 mb-1">Contact
                                    (Email/Phone)</label>
                                <input
                                    id="contact"
                                    name="contact"
                            ...
                            <div>
                                <label htmlFor="message" className="block text-sm font-medium text-gray-300 mb-1">Message
                                    (Optional)</label>
                                <textarea
                                    id="message"
                                    name="message"
                            ...
                            {error && <div
                                className="text-red-400 text-sm bg-red-900/30 border border-red-700 p-3 rounded-md">{error}</div>}
                            <div className="flex flex-col sm:flex-row gap-3 mt-3">
                                ...
                                <LoadingButton
                                    ...
                                    disabled={loading}
                                    variant="secondary"
                                    className="px-6 py-3 bg-gray-600 hover:bg-gray-500 text-gray-200"
                                >
                                    Cancel
                                </LoadingButton>
                            </div>
    ...
    return (
        <div className="fixed inset-0 bg-black/30 flex items-center justify-center z-50">
            <div
                className="bg-white rounded-xl p-6 border-2 border-pink-100 flex flex-col gap-3 min-w-[350px] max-w-[90vw]">
                <h2 className="text-lg font-bold text-pink-700 mb-2">Request: {offering.title}</h2>
                <form className="flex flex-col gap-3" onSubmit={handleSubmit}>
                    <input name="name" value={form.name} onChange={handleChange} placeholder="Your Name"
                           className="input-field border rounded px-2 py-1" required/>
                    <input name="contact" value={form.contact} onChange={handleChange}
                           placeholder="Contact (email/phone)" className="input-field border rounded px-2 py-1"
                           required/>
                    <textarea name="message" value={form.message} onChange={handleChange}
                              placeholder="Message (optional)"
                              className="input-field border rounded px-2 py-1 min-h-[40px]"/>
                    <div className="flex gap-2 mt-2">
                        <button type="submit" className="interactive-element px-3 py-1 bg-pink-500 text-white rounded"
                                disabled={loading}>Send Request
                        </button>
                        <button type="button" className="interactive-element px-3 py-1 bg-gray-200 rounded"
                                onClick={onClose} disabled={loading}>Cancel
                        </button>
                    </div>
                    {success &&
                        <div className="text-green-600 text-sm mt-1">Request sent! Ananya will contact you soon.</div>}
