...
const OfferingCard = ({offering, onDelete, onEdit, globalDiscount = 0, showActions = true}) => {
    ...
    return (
        <div
            className="bg-slate-800 border border-slate-700 rounded-xl shadow-lg p-5 flex flex-col items-center gap-3 w-72 hover:shadow-sky-500/20 transition-shadow duration-300 max-h-[480px]">
            <img
            ...
            <div className="flex flex-col items-center gap-2 overflow-y-auto w-full px-1 flex-grow">
                <h3 className="text-xl font-semibold text-sky-400 mb-1 text-center truncate w-full flex-shrink-0"
                    title={offering.title}>{offering.title}</h3>
                ...
                {offering.description &&
                    <p className="text-slate-400 text-sm mb-2 text-center ">{offering.description}</p>}
                {discountBanner}
            </div>
            {showActions && (
