...
import { HiOutlineSparkles, HiXMark, HiCheckCircle, HiUser, HiEnvelope } from "react-icons/hi2";
import apiService from '../utils/apiService';

// TODO: Implement backend API endpoint to handle similar event requests

const RequestSimilarEventModal = ({ event, onClose }) => {  const [form, setForm] = useState({ 
    name: "", 
    email: "", 
    phone: "", 
    message: "" 
      ...
      >
        ...
        <div className="text-center mb-6">
          ...
          <p className="text-slate-400 text-sm">
            Inspired by <span className="text-sky-300 font-semibold">"{event?.title}"</span>? Let's create something amazing for you!
          </p>
        </div>
        ...
        {success ? (
          <div className="text-center py-8">
            ...
            <p className="text-slate-400 mb-6">
              Thank you for your interest! Our event planning team will contact you within 24 hours to discuss your vision for a similar event.
            </p>
            <button
              onClick={onClose}
              className="px-8 py-3 bg-sky-600 hover:bg-sky-700 text-white rounded-lg font-semibold transition-colors duration-200 shadow-lg"
            >
              Close
            </button>
          </div>
            ...
            <div>
              ...
              <input 
                ...
                className="w-full bg-slate-700 border border-slate-600 text-slate-200 rounded-lg px-4 py-3 focus:ring-2 focus:ring-sky-500 focus:border-sky-500 outline-none transition-colors duration-200"
                required 
              />
            </div>
            ...
            <div>
              ...
              <textarea 
                ...
                rows={4}
                className="w-full bg-slate-700 border border-slate-600 text-slate-200 rounded-lg px-4 py-3 focus:ring-2 focus:ring-sky-500 focus:border-sky-500 outline-none transition-colors duration-200 resize-y min-h-[100px]"
                required
              />
            </div>
            ...
            <div className="flex flex-col sm:flex-row gap-3 pt-4 border-t border-slate-700">
              <button
                type="submit"
                disabled={loading}
                className="flex-1 px-6 py-3 bg-sky-600 hover:bg-sky-700 text-white font-semibold rounded-lg shadow-md transition-colors duration-200 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
              >
                {loading ? (
                  <>
              ...
              <button
                type="button"
                onClick={onClose}
                className="px-6 py-3 bg-slate-600 hover:bg-slate-500 text-slate-300 font-semibold rounded-lg shadow-md transition-colors duration-200 border border-slate-500 hover:border-slate-400"
                disabled={loading}
              >
                Cancel
              </button>
