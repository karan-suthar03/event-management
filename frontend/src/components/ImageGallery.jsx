...
const ImageGallery = ({
    ...
    if (!images || images.length === 0) {
        return (
            <div
                className={`bg-slate-700 rounded-xl border border-slate-600 aspect-video flex items-center justify-center ${className}`}>
    ...
    return (
        <div className={`image-gallery ${className}`}>
            <div
                className="relative overflow-hidden rounded-xl border-2 border-slate-700 hover:border-sky-500 transition-all duration-300">
                <div
                ...
                />
                    {enableLightbox && (
                        <div
                            className="absolute top-3 right-3 bg-black/50 p-2 rounded-full hover:bg-sky-600/70 transition-colors">
                    ...
                    <div className="relative w-full max-w-6xl h-full max-h-[90vh]" onClick={e => e.stopPropagation()}>
                        ...
                        <div className="relative h-full flex items-center justify-center">
                            ...
                            {showCaptions && getImageCaption(images[currentIndex]) && (
                                <div
                                    className="absolute left-0 right-0 bottom-0 bg-black/60 text-white p-4 text-center">
                        ...
                        <div
                            className="absolute bottom-4 left-1/2 -translate-x-1/2 max-w-lg overflow-x-auto flex gap-2 px-2 py-2 bg-black/50 rounded-lg hide-scrollbar">
                            {images.map((img, idx) => (
                                <div
                                    key={idx}
                                    onClick={() => setCurrentIndex(idx)}
