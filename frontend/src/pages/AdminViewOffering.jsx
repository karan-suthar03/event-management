...
const AdminViewOffering = () => {
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            if (offering.categories) {
                fd.append("categories", JSON.stringify(offering.categories.map(c => ({
                    id: c.id,
                    name: c.name,
                    emoji: c.emoji
                }))));
            ...
            if (offering.mainCategory) {
                fd.append("mainCategory", JSON.stringify({
                    id: offering.mainCategory.id,
                    name: offering.mainCategory.name,
                    emoji: offering.mainCategory.emoji
                }));
            }
            ...
            <div className="flex flex-col items-center justify-center p-4">
                <div className="bg-red-900/30 border border-red-700 p-6 rounded-lg shadow-xl text-center">
                    ...
                    <button onClick={() => navigate(-1)}
                            className="interactive-element mt-6 px-4 py-2 bg-sky-600 hover:bg-sky-700 text-white rounded-lg font-medium transition-all">Go
                        Back
                    </button>
                </div>
            </div>
    ...
    if (loading) return <div className="flex items-center justify-center">
        ...
        <p className="ml-3 text-sky-400">Loading offering details...</p></div>;
    if (error) return <div className="flex flex-col items-center justify-center p-4">
        <div className="bg-red-900/30 border border-red-700 p-6 rounded-lg shadow-xl text-center"><p
            className="text-red-400 font-semibold text-lg">Error Loading Offering</p><p
    ...
    if (!offering) return <div className="flex items-center justify-center"><p
        className="text-slate-400 text-xl">Offering not found.</p></div>;
            ...
            <div className="max-w-3xl mx-auto bg-slate-800 rounded-xl shadow-2xl border border-slate-700 p-6 md:p-10">
                <header className="flex items-center justify-between mb-8 pb-6 border-b border-slate-700">
                    <div>
                        ...
                        <p className="text-slate-400 mt-1">Update details for the offering: <span
                            className='text-sky-300 font-semibold'>{offering.title}</span></p>
                    </div>
                    <button
                ...
                <div className="space-y-8">
                    ...
                    <section className="text-center p-6 bg-slate-850/50 rounded-lg border border-slate-700">
                        ...
                        {newImagePreview &&
                            <p className="text-xs text-slate-400 mt-2">New photo selected. Click "Save Changes" to
                                apply.</p>}
                    </section>
                    ...
                    <section className="p-6 bg-slate-850/50 rounded-lg border border-slate-700 space-y-6">
                        <div className="flex justify-between items-center border-b border-slate-700 pb-3 mb-4">
                            ...
                            <button
                                type="button"
                                className="interactive-element px-4 py-2 rounded-lg bg-sky-600 hover:bg-sky-700 text-white text-xs font-semibold transition-all flex items-center gap-1.5"
                        ...
                        <div>
                            <label className="block text-sm font-medium text-sky-300 mb-2">Select Categories
                                (Multiple)</label>
                      ...
                      ${offering.categories && offering.categories.some(c => c.id === cat.id)
                        ...
                        <div>
                            <label className="block text-sm font-medium text-sky-300 mb-2">Select Main Category
                                (Single)</label>
                            <div className="flex flex-wrap gap-2">
                        ...
                        {!editingInclusions ? (
                            ...
                            <div className="space-y-3">
                                ...
                                <div className="flex gap-3 pt-2">
                                    <button
                                        type="button"
                                        className="interactive-element px-4 py-2 rounded-lg bg-sky-600 hover:bg-sky-700 text-white font-semibold text-sm transition-all flex items-center gap-1.5"
                                    ...
                                    <button
                                        type="button"
                                        className="interactive-element px-4 py-2 rounded-lg bg-green-600 hover:bg-green-700 text-white font-bold shadow-md hover:shadow-lg transition-all duration-300 ease-in-out flex items-center gap-2 disabled:opacity-60"
                    ...
                    <section className="p-6 bg-slate-850/50 rounded-lg border border-slate-700 space-y-6">
                        ...
                        {offering.discountType === 'SPECIAL' && (
                            <div className="mb-4">
                                <label className="block text-sm font-medium text-sky-300 mb-1">Specific Discounted Price
                                    (₹)</label>
                                <input
                                    type="number"
                                    min="0"
                        ...
                        <div className="text-slate-400 text-sm">
                            <span
                                className="font-semibold">Current:</span> {offering.discountType === 'GLOBAL' ? 'Global Discount' : offering.discountType === 'SPECIAL' ? `Special Discount (₹${offering.specificDiscountedPrice || 0})` : 'No Discount'}
                        </div>
                    </section>
