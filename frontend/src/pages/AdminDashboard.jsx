...
const AdminDashboard = () => {
    ...
    useEffect(() => {
        ...
        apiService.get("/api/global-discount", false)
            .then(data => {
                ...
                setDiscountInput(data.discount || '0');
            })
            .catch(() => setGlobalDiscount(0));
    ...
    const handleDelete = async (id) => {
        if (window.confirm("Are you sure you want to delete this event?")) {
            try {
                // Make API call to delete the event from the server
    ...
    const handleDeleteFeedback = async (id) => {
        if (window.confirm("Are you sure you want to delete this feedback?")) {
            try {
                // Make API call to delete the feedback from the server
                await apiService.delete(`/api/feedbacks/${id}`, true);
                // Remove the feedback from local state after successful deletion
    ...
    const handleDiscountSave = async () => {
        ...
        setDiscountError("");
        try {
            // Convert to number before sending to API, handling empty strings
                ...
                <section className="mb-8 bg-slate-800/50 p-6 rounded-lg border border-slate-700 shadow-md">
                    ...
                    <div className="flex flex-col sm:flex-row gap-4 items-start sm:items-end">
                        ...
                        <button
                            ...
                            className="interactive-element px-6 py-2 bg-sky-600 hover:bg-sky-700 disabled:bg-slate-600 text-white rounded-lg font-medium transition-all duration-200 flex items-center gap-2"
                        >
                            {discountLoading ? (
                                <>
                                    <div
                ...
                <section className="mb-12 bg-slate-800/50 p-6 rounded-lg border border-slate-700 shadow-md">
                    <div className="flex flex-wrap items-center justify-between gap-4 mb-6">
                        ...
                        <div className="flex flex-wrap gap-3">
                            <button
                                className="interactive-element px-5 py-2.5 bg-sky-600 hover:bg-sky-700 text-white rounded-lg shadow font-medium text-sm transition-all duration-200 flex items-center gap-2"
                                onClick={handleAdd}
                            >
                            ...
                            <button
                                className="interactive-element px-5 py-2.5 bg-slate-600 hover:bg-slate-500 text-sky-300 rounded-lg shadow font-medium text-sm transition-all duration-200 border border-slate-500 hover:border-sky-400"
                    ...
                    {error && (
                        <div className="text-center py-8 bg-red-900/30 rounded-lg border border-red-700">
                            ...
                            <button
                                ...
                                className="interactive-element px-6 py-2.5 bg-red-600 hover:bg-red-700 text-white rounded-lg font-medium transition-all inline-flex items-center gap-2"
                            >
                    ...
                    {!loading && !error && events.length === 0 ? (
                        <div className="text-center py-12">
                            ...
                            <button
                                onClick={handleAdd}
                                className="interactive-element px-6 py-3 bg-sky-600 hover:bg-sky-700 text-white rounded-lg font-medium transition-all inline-flex items-center gap-2"
                            >
                        ...
                        <div className="overflow-x-auto rounded-lg border border-slate-700">
                            <table className="w-full text-left text-sm">
                                ...
                                {pagedEvents.length === 0 ? (
                                    <tr>
                                        <td colSpan="5" className="text-center py-8 text-slate-400 text-base">No events
                                            found.
                                        </td>
                                    </tr>
                                ) : pagedEvents.map(ev => (
                                    <tr key={ev.id}
                                        className="bg-slate-800 hover:bg-slate-700/50 transition-colors duration-150">
                                        ...
                                        <td className="py-3 px-4 text-center">
                                            <button
                                                className={`interactive-element px-3 py-1 rounded-full text-xs font-semibold shadow-sm transition-all ${ev.featured ? 'bg-yellow-500 text-yellow-900 hover:bg-yellow-400' : 'bg-slate-600 text-slate-300 hover:bg-slate-500'}`}
                                        ...
                                        <td className="py-3 px-4 flex items-center justify-center gap-2 whitespace-nowrap">
                                            <button
                                                className="interactive-element p-2 bg-sky-600 hover:bg-sky-700 text-white rounded-md text-xs transition-all"
                                                onClick={() => handleEdit(ev.id)} title="Edit Event">
                                                <HiPencil/>
                                            </button>
                                            <button
                                                className="interactive-element p-2 bg-red-600 hover:bg-red-700 text-white rounded-md text-xs transition-all"
                                                onClick={() => handleDelete(ev.id)} title="Delete Event">
                                                <HiTrash/>
                                            </button>
                                        </td>
                                    </tr>
                    ...
                    {!loading && !error && events.length > EVENTS_PER_PAGE && (
                        <div
                            className="flex items-center justify-center gap-3 mt-6">                        {page > 1 &&
                            <button
                                className="interactive-element px-4 py-2 bg-slate-600 hover:bg-slate-500 text-sky-300 rounded-md text-sm font-medium transition-all"
                    ...
                    <div className="space-y-5">
                        {lastTenFeedbacks.length === 0 && !loading ? (
                            ...
                            lastTenFeedbacks.map(fb => (
                                <div key={fb.id}
                                     className="bg-slate-700/60 border border-slate-600 rounded-lg p-5 shadow-sm transition-all hover:shadow-md hover:border-slate-500">
                                    <div className="flex items-start justify-between mb-2">
                                        <div>
                                            <span className="font-semibold text-sky-400 text-base mr-2">{fb.name}</span>
                                            {fb.eventTitle && (
                                                <span className="text-xs text-slate-400">on <button
                                                    onClick={() => navigate(`/events/${fb.eventId}`)}
                                                    disabled={!fb.eventId}
                                                    className="interactive-element text-sky-500 hover:underline disabled:text-slate-500 disabled:no-underline">{fb.eventTitle}</button></span>
                                            )}
                                        </div>
                                        <span
                                            className="text-xs text-slate-500 flex-shrink-0 ml-2">{fb.date || new Date().toLocaleDateString()}</span>
                                    </div>
                                    <p className="text-slate-300 text-sm mb-3 leading-relaxed">{fb.message}</p>
                                    <div
                                        className="flex items-center justify-end gap-2 mt-2 pt-3 border-t border-slate-600/50">
                                        <button
                                            className="interactive-element p-2 bg-red-600 hover:bg-red-700 text-white rounded-md text-xs transition-all"
