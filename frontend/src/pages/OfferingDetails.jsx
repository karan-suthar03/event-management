...
const OfferingDetails = ({ globalDiscount = 0 }) => {
  ...
  if (loading) {
    return <div className="min-h-screen flex items-center justify-center text-sky-400 text-xl">Loading package...</div>;
  }
  if (error || !offering) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center">
        <h2 className="text-3xl font-bold text-red-400 mb-4">Offering Not Found</h2>
        <button onClick={() => navigate('/offerings')} className="px-6 py-2 bg-sky-600 text-white rounded-full font-bold shadow hover:bg-sky-700 transition">Back to All Offerings</button>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen p-4 md:p-8 flex flex-col items-center">
      <div className="w-full max-w-4xl mx-auto bg-slate-800 rounded-2xl shadow-2xl border border-slate-700 p-6 md:p-10 mt-8">
        <button
          ...
          <div className="md:col-span-1 space-y-6">
            <div className="relative">
              {offering.images && offering.images.length > 0 ? (
                ...
                <div className="w-full h-56 flex items-center justify-center bg-slate-800 rounded-xl border border-slate-700">
                  <p className="text-slate-400">No image available</p>
                </div>
              )}
              {offering.bestSeller && (
                <span className="absolute top-2 right-2 px-3 py-1 text-xs font-bold rounded-full shadow bg-sky-500 text-sky-100 z-10">Best Seller</span>
              )}
            </div>
            <div className="bg-slate-900 p-5 rounded-xl shadow border border-slate-700 space-y-2">
              <div className="flex items-center space-x-2">
                ...
                <span className="text-lg font-semibold text-sky-400">{offering.mainCategory?.name || "Package"}</span>
              </div>
              {discountBanner}
          ...
          <div className="md:col-span-2 flex flex-col gap-8">
            ...
            <div className="flex flex-wrap gap-4 pt-4 items-center justify-start border-t border-slate-700 mt-8">
              <button
                ...
                className="flex items-center px-6 py-3 bg-sky-600 hover:bg-sky-700 text-white font-semibold rounded-lg shadow-md transition-colors duration-300 border border-sky-500 hover:border-sky-400 focus:outline-none focus:ring-2 focus:ring-sky-500 focus:ring-opacity-50"
              >
